...
public class EtudiantDAO extends ConnectionDAO {
	...
	public int add(Etudiant etudiant) {
		...
		try {
			...
			ps = con.prepareStatement("INSERT INTO supplier(id, name, surname, email, promo) VALUES(?, ?, ?, ?,?)");
	...
	public int update(Etudiant etudiant) {
		...
		try {
			...
			// les getters permettent de recuperer les valeurs des attributs souhaites
			ps = con.prepareStatement("UPDATE etudiant set name = ?, surname = ?, email = ?, promo = ? WHERE id = ?");
	...
	public Etudiant get(int id) {
		...
		try {
			...
			if (rs.next()) {
				returnValue = new Etudiant(rs.getInt("id"),
									       rs.getString("nom"),
									       rs.getString("prenom"),
									       rs.getString("email"),
				                           rs.getString("promo"));
	...
	public ArrayList<Etudiant> getList() {
		...
		try {
			...
			while (rs.next()) {
				returnValue.add(new Etudiant(rs.getInt("id"),
						                     rs.getString("nom"),
						                     rs.getString("prenom"),
						                     rs.getString("email"),
						                     rs.getString("promo")));
	...
	public static void main(String[] args) throws SQLException {
		...
		Etudiant e1 = new Etudiant(ids[0], "Amadou", "Mbodj", "monfournisseurprincipal@mail.com","1ère année");
		Etudiant e2 = new Etudiant(ids[1], "Lala", "Diallo", "monfournisseursecondaire@mail.com","1ère année");
		Etudiant e3 = new Etudiant(ids[2], "RORO", "Rety", "monfournisseursecours@mail.com","1 ère année");
